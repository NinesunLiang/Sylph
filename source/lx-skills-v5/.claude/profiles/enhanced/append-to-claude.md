...
```
plan
_gate: true — Research Gate → Plan Gate 两阶段强制门禁（已自动开启）lx-rpe — 模式一：大模块特性完整生命周期lx-todo — 模式二：小任务随时捕获与处理lx-task-spec — 模式三：重要/复杂任务严格 Spec 驱动
...
```
任务规模 ≤3 文件 且 边界清晰？ → 是：/lx-rpe todo add ... 或 /lx-todo add ...（模式二）
...
```
1. /lx-rpe new → 创建 rpe/{feature}/ 目录结构 → 填写 prd.md（或 AI 引导式收集） → AI 自动生成 research.md 草稿（关键调用链 + 风险）
...
yamltask_
in
put: task_name: "refactor-auth-module" target: "将 auth 模块从 monolith 拆分为独立 service" pass_criteria: - id: AC-1 description: "所有现有 auth 测试通过" how_to_check: "go test ./pkg/auth/..." expected: "PASS" executor_mode: stepwise priority: p0
