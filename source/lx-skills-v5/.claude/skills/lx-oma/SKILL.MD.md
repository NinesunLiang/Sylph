...
bash
mkdir
-p rpe/feat-XXXecho "# [Feature] XXX 的需求" > rpe/feat-XXX/prd.mdecho "# 技术调研" > rpe/feat-XXX/research.mdecho "# 实施计划" > rpe/feat-XXX/plan.md
...
## 🚀 并发启动指令请打开 N 个终端，分别运行以下指令开始并发构建：\\`\\`\\`bash# 终端 1/lx-rpe feat-xxx# 终端 2/lx-rpe feat-yyy\\`\\`\\`底层的 OMA 文件锁 (Micro-OS Mutex) 已就绪，冲突将自动挂起排队，尽情享受最高密度的并发生产力！
```
