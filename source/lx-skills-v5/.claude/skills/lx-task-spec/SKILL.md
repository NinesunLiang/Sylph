...
**无参数时的恢复逻辑**：
1. 检查 `.omc/state/` 是否有活跃 task-spec 状态
2. 有 → 输出当前进度摘要 + 直接恢复执行，不打断
3. 无 → 提示："当前无活跃任务，使用 `/lx-task-spec <描述>` 创建新任务"
以下情况也触发本 skill：
- 用户说"启动 task_spec" / "用 task_spec 处理 XXX"（有任务描述 → 视为有参数）
- lx-todo 升级（超限自动跳转，携带 todo 上下文）
**三种模式边界**：
| 模式 | 适用场景 | 升级方向|
|------|---------|---------|
...
1. 加载 `@../../task_sys/loading_matrix.md` → 了解加载映射
2. 加载 `@../../task_sys/orchestrator.md` → 启动状态机
3. 根据 Orchestrator 路由结果，按需加载对应节点
...
```
问题2/3：要做什么？（描述具体需求，如 "在用户模块新增登录接口，支持手机号+验证码"）
...
```
问题3/3：如何验证任务完成？（描述可观测的验收标准，如 "
1. 接口返回 200 2. 单元测试通过 3. 文档更新"）不确定？直接说"帮我生成"
...
```
生成规则：
- AC-1 对应 Q2 描述的核心功能（必有）
- AC-2 对应测试验证（必有，类型按项目类型：Go→go test / 前端→npm test）
- AC-3 对应 Q2 中能识别的边界场景（可选，识别不到则省略）
- 所有 AC 必须可观测、可执行，不接受"代码合理"这类主观描述
>
...
**task_spec 渐进式披露规则**：
- `AGENTS.md` / `CLAUDE.md` 中 task_spec 相关的普通路径只是索引，不会自动触发加载。
- 只有明确触发本 skill 后才开始加载。
- 触发本 skill 后，按 `loading_matrix.md` 的映射表，**仅加载当前阶段需要的文件**。

## 状态机

```
js引导
收集→ need_clarification → ready → planning → executing → done ↑ ↑ ↑ ↑ └────── blocked ────────┘ └─ fallback ┘ ↓ need_clarification
