...
- 过滤：保留 `*.tsx`、`*.jsx`。排除：`*.test.tsx`、`*.test.jsx`、`*.stories.tsx`、`*.spec.tsx`、`*.d.ts`、`node_modules/`、`.next/`、`dist/`、`build/`
...
mark
down##
[React Review 反哺] {规则号} {问题简述}
- **文件**: `{file:line}` - **问题**: {问题描述} - **改进**: {修复方式}
- **来源**: lx-react-review auto-fix - **严重度**: {P0 🔴 / P1 🟠}
...
- 过滤后无 React 文件 → "无 React 变更"报告
- 非 React 项目（无 react 依赖在 package.json）→ "不适用"
- 全部命中为误报 → "通过"报告
