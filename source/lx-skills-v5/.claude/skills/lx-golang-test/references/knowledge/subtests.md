...
- Logically grouped test scenarios
- Independent tests that can run in parallel
- Shared setup with `t.Cleanup()`

## Rules

- `t.Parallel()` only for truly independent tests
- Never parallel if tests share mutable state
- Go < 1.22: capture range var with `tt := tt`
- Use `t.Cleanup()` for teardown, not `defer` in subtests
