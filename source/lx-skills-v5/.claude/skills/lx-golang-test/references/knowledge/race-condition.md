...
- Use `sync.WaitGroup`, channels, or `context.WithTimeout`
- Always run `-race` for concurrent code
- Use `-count=N` to stress-test race conditions
- Auto-append race tests when function touches shared state
