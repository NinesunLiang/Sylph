...
```
gofunc BenchmarkXxx(b *testing.B) { // Setup outside loop input := prepareInput()
...
- Prevent compiler optimization: assign to package-level `var result`
- Run multiple times with `-count=5` for stable results
