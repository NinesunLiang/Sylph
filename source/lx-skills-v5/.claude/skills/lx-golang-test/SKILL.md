...
**测试生成原则**：
- Table-Driven Tests：每个 case 独立命名、独立断言
- Mock-based Tests：只 Mock 外部依赖，不 Mock 被测函数
- HTTP Handler：测试完整请求/响应周期
- Benchmark：包含 `b.ResetTimer()` 和 `b.ReportAllocs()`
...
```
bashp
ython3 .claude/skills/lx-golang-test/scripts/run_go_tests.py \ --pkg {target_pkg} --race --cover
...
验证序列：
1. 编译：`go build ./...`
2. 测试：`go test ./target/pkg/... -v -count=1`
3. Race 检测：`go test -race ./target/pkg/...`
4. 覆盖率：`go test -cover ./target/pkg/...`
