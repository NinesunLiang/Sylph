...
if [ -z "$NEW_VER" ]; then
    ...
    echo "  未指定版本，自动升为 v${NEW_VER}-stable"
fi

check 1 "更新 VERSION.json → ${NEW_VER}"
sed -i.bak "s/\"version\": \"${VER}\"/\"version\": \"${NEW_VER}\"/" VERSION.json
rm -f VERSION.json.bak
python3 -c "import json; v=json.load(open('VERSION.json'))['version']; exit(0 if v=='${NEW_VER}' else 1)" && pass || fail "更新失败"

check 2 "更新 harness.yaml harness_version → ${NEW_VER}"
sed -i.bak "s/harness_version: ${VER}/harness_version: ${NEW_VER}/" .claude/harness.yaml
rm -f .claude/harness.yaml.bak
grep -q "harness_version: ${NEW_VER}" .claude/harness.yaml && pass || fail "更新失败"
