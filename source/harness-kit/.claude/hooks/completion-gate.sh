...
try:
    ...
    if [ "$FRESH" = "yes" ]; then
        ...
        DUAL_SOURCE=$(echo "$CONTENT" | ${PYTHON_BIN:-python3} -c "
import sys, re
c = sys.stdin.read()
sources = 0
set -f
...
" 2>/dev/null)
        ...
        QUALITY_SCORE=$(echo "$CONTENT" | ${PYTHON_BIN:-python3} -c "
import sys, re
...
" 2>/dev/null)
	...
	_SCORE_NUM=$(echo "$QUALITY_SCORE" 2>/dev/null | head -1 | grep -oE '^[0-9]+' || echo "0")
        ...
        if [ -n "$_SCORE_NUM" ] && [ "$_SCORE_NUM" -lt "$QUALITY_THRESHOLD" ] 2>/dev/null; then
            ...
            echo "质量分解与改进方向:" >&2
            cat "$CONSUMED" 2>/dev/null | ${PYTHON_BIN:-python3} -c "
import sys, re
content = sys.stdin.read()
...
" 2>/dev/null
        ...
        # Karpathy-E5: test-first 复现证据 — 修复类任务必须含复现失败的证据（karpathy-guidelines#4）
        ...
        echo "$RCA_CONTENT" | grep -qiE '(repro|复现|触发条件).{5,}' && { RCA_STRUCTURED=$((RCA_STRUCTURED + 1)); RCA_HAS_REPRO=1; }
        ...
        # Karpathy test-first: 复现证据必须包含实际失败输出，不只是提到"repro"
        # 检测修复类任务是否包含复现证据（FAIL/exit non-zero/错误信息）
        RCA_REPRO_EVIDENCE=0
        if [ "$RCA_HAS_REPRO" -eq 1 ]; then
            echo "$RCA_CONTENT" | grep -qiE '(FAIL|exit.*[1-9]|Traceback|Error:|assertion.*fail|失败输出|复现命令)' && RCA_REPRO_EVIDENCE=1
        fi
        ...
        if [ "$RCA_STRUCTURED" -ge 2 ]; then
            ...
            if [ -n "$QS_VALUE" ] && [ "$QS_VALUE" -ge "$QS_THRESHOLD_NUM" ] 2>/dev/null; then
                ...
                if [ "$RCA_TEMPLATED" -ge 1 ]; then
                    ...
                    # Karpathy test-first: 如果有 repro 关键词但无实际复现证据，额外提示
                    if [ "$RCA_HAS_REPRO" -eq 1 ] && [ "$RCA_REPRO_EVIDENCE" -eq 0 ]; then
                        echo "  ⚠️ [test-first] 复现字段存在但缺少实际失败输出（FAIL/exit非零/Traceback），请补充复现命令+输出。" >&2
                    fi
                    rm -f "$CONSUMED"
