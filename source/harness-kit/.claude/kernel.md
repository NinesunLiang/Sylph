...
- ✅ 不再新增行为铁律、Gate 类型- ✅ 所有新经验默认进入 `.claude/claude-next.md`，验证稳定后可升华到此文件
...
## 测试要求
<!-- 由 R17 审计填充 @2026-05-07 -->
- **Harness Smoke**：修改 hook 后必须通过 `harness-smoke-test.sh`（57 cases）
- **Hook 生产验证**：`hook-production-verify.sh`（25 cases）覆盖所有 gate 场景
