#!/usr/bin/env python3
"""
lx-orch-status.py — Pipeline Orchestrator 状态面板
Python 移植版，完全等价 lx-orch-status.sh v1.0

从 governance-spec.md §Pipeline 集成 规范实现
用法: python3 lx-orch-status.py
"""
...
def main():
    if not PIPELINE_FILE.exists():
        print(f"ERROR: pipeline.yaml 未找到 ({PIPELINE_FILE})", file=sys.stderr)
        print("提示: 请先运行 /lx-oma-gov init 创建治理目录", file=sys.stderr)
        sys.exit(1)
    ...
    for s in STAGE_ORDER:
        ...
        icon = {'completed': '✅', 'initialized': '🟡', 'running': '🔄',
                'pending': '⬜', 'blocked': '🔴'}.get(status, '⬜')
    ...
    for sub in subs:
        oracle_icon = {'final': '✅', 'approved': '✅', 'revised': '⚠️', 'pending': '⬜'}.get(
            sub.get('oracle', ''), '⬜')
    ...
    # Always available
    available_actions.append(("/lx-orch status", "刷新本面板"))
    available_actions.append(("自定义操作", "输入你想做的其他操作（如直接调 skill / 手动检查文件）"))
