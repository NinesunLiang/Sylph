...
def before_compact(state_dir: str):
    ...
    优先从 .last-user-prompts 读取最近 query（CC turn-counter 写入），
    回退到 todo-queue.md（旧格式兼容）。
    """
    todo_file = os.path.join(state_dir, 'todo-queue.md')
    prompt_log = os.path.join(state_dir, '.last-user-prompts')
    user_queries = []
    # 新格式：.last-user-prompts（时间戳||query 环形队列）
    if os.path.isfile(prompt_log):
        try:
            with open(prompt_log, encoding='utf-8') as f:
            ...
            for line in lines[-20:]:
                ...
                if '||' in line:
                    _, q = line.split('||', 1)
                    user_queries.append(q.strip()[:120])
                else:
                    user_queries.append(line[:120])
        except Exception:
            pass
    # 回退：旧格式 todo-queue.md
    if not user_queries:
