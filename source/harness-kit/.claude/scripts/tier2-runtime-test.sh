...
_test "completion-gate exists" "true" "$([ -f $H/completion-gate.py ] && echo true)"
_test "posttool-completion-audit exists" "true" "$([ -f $H/posttool-completion-audit.py ] && echo true)"
_test "both registed in settings.json" "true" "$(
  c1=$(grep -c 'completion-gate.py' .claude/settings.json 2>/dev/null || echo 0)
  c2=$(grep -c 'posttool-completion-audit.py' .claude/settings.json 2>/dev/null || echo 0)
  [ "$c1" -gt 0 ] && [ "$c2" -gt 0 ] && echo true
)"

# [20] permission-gate + privacy-gate — 双层拦截
echo ""; echo "=== [20] permission-gate + privacy-gate ==="
_test "permission-gate exists" "true" "$([ -f $H/permission-gate.py ] && echo true)"
_test "privacy-gate exists" "true" "$([ -f $H/privacy-gate.py ] && echo true)"
...
_test "scope hook exists" "true" "$([ -f $H/pretool-edit-scope.py ] && echo true)"
_test "tracker hook exists" "true" "$([ -f $H/intent-tracker.py ] && echo true)"
_test "tracker has E6 fix" "true" "$(grep 'contradiction_level >= 2' $H/intent-tracker.py | grep -c '2')"
...
_test "error-dna exists" "true" "$([ -f $H/error-dna.py ] && echo true)"
...
_test "compressor exists" "true" "$([ -f $H/context-compressor.py ] && echo true)"
...
_test "lsp-suggest exists" "true" "$([ -f $H/lsp-suggest.py ] && echo true)"
_test "pre-edit-lsp exists" "true" "$([ -f $H/pre-edit-lsp-check.py ] && echo true)"
_test "both registered in settings" "true" "$(
  l1=$(grep -c 'lsp-suggest.py' .claude/settings.json 2>/dev/null || echo 0)
  l2=$(grep -c 'pre-edit-lsp-check.py' .claude/settings.json 2>/dev/null || echo 0)
...
_test "meta-oracle-trigger exists" "true" "$([ -f $H/meta-oracle-trigger.py ] && echo true)"
...
_test "permission-gate catches danger" "true" "$(
  echo '{"tool_name":"Bash","tool_input":{"command":"echo test"}}' | \
    bash $H/permission-gate.py 2>/dev/null | \
