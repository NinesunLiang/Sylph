...
def main():
    ...
    # === E6 编辑矛盾信号检测（v2 — 机制升级：不依赖 intent-tracker 的 contradiction 标记）===
    # 逻辑：从 edit-churn-log.jsonl 中独立分析编辑模式
    # 1. edit_repeat：同一文件编辑 3+ 次（重复编辑信号）
    # 2. content_flip：连续编辑中内容 hash 多次变化（改→回→改）
    # 3. revert_detected：内容回退到历史版本
    # 所有检测基于日志中的 stats（edit_count, content_hash），不依赖 AI 自标记
    E6_VIOLATIONS = ''
    ...
    if CONTRADICTION_LOG.is_file() and FILE_PATH:
                ...
                for line in f:
                    if FILE_PATH in line:
                        try:
                            ...
                            pass
            if len(matching) >= 2:
                reversions = [r for r in matching if r.get('revert_of') is not None]
                edit_counts = sorted([r.get('edit_count', 0) for r in matching], reverse=True)
                max_edit_count = edit_counts[0] if edit_counts else 0
                sig_count = len(set(r.get('sig', '') for r in matching))

                # E6.1: revert 检测（最明确的矛盾信号）
                if reversions:
                    E6_CHECK_parts = [f"[E6] REVERT_DETECTED: {FILE_PATH} — {len(reversions)} 次内容回退"]
                    for r in reversions[:2]:
                        E6_CHECK_parts.append(f"  · revert_of={str(r.get('revert_of',''))[:16]}... ts={r.get('ts','?')}")
                    E6_CHECK = '\n'.join(E6_CHECK_parts)

                # E6.2: 高频编辑（edit_count ≥4 = 明确重复编辑矛盾信号）
                if not E6_CHECK and max_edit_count >= 4:
                    E6_CHECK_parts = [f"[E6] EDIT_REPEAT: {FILE_PATH} — 编辑 {max_edit_count} 次, {sig_count} 个签名, 文件在第 {max_edit_count} 次编辑仍被修改"]
                    E6_CHECK_parts.append(f"  · 频繁修改同一文件 — 设计/实现可能未固化，建议检查编辑历史")
                    E6_CHECK = '\n'.join(E6_CHECK_parts)

                # E6.3: 多次编辑 + 内容 hash 变化检测（content_flip 模式）
                if not E6_CHECK and max_edit_count >= 3 and len(matching) >= 3:
                    # 检查最近 3 条记录的 content_hash 变化模式
                    recent = sorted(matching, key=lambda x: x.get('ts', 0), reverse=True)[:3]
                    hashes = [r.get('content_hash', '') for r in recent if r.get('content_hash')]
                    if len(hashes) >= 3 and len(set(hashes)) == 3:
                        # 连续 3 次编辑内容 hash 都不同 → 方向变化信号
                        E6_CHECK_parts = [f"[E6] CONTENT_FLIP: {FILE_PATH} — 连续 3 次编辑内容均不同，编辑方向未收敛"]
                        E6_CHECK_parts.append(f"  · 建议在确定设计后再修改，避免反复切换方案")
                        E6_CHECK = '\n'.join(E6_CHECK_parts)

            # 非 violation 的高频编辑只打印到 stderr
            if not E6_CHECK and max_edit_count > 5 and sig_count > 2:
                print(f"[E6] HIGH_CHURN: {FILE_PATH} — {len(matching)} 条编辑, {sig_count} 个签名, 最高 {max_edit_count} 次 (非矛盾)", file=sys.stderr, flush=True)
        ...
        if E6_CHECK:
            ...
            flywheel_event('posttool_claim_audit', 'e6_contradiction_detected', 'P2')
