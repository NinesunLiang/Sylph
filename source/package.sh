...
mkdir -p "$HK_TMP/.claude/hooks" "$HK_TMP/.claude/scripts"
...
find "$HK_SRC/.claude/scripts/" -maxdepth 1 -type f -exec cp -f {} "$HK_TMP/.claude/scripts/" \;
chmod +x "$HK_TMP/.claude/hooks/"*.sh

# 清理 pycache
find "$HK_TMP" -name "__pycache__" -type d -exec rm -rf {} + 2>/dev/null || true
...
log_info "✅ 打包完成！$VERSION — packages/ 目录已就绪"
