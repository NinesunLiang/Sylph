...
PRETOOL_GATES = [
    "hermes-pre-exec.sh", "hermes-completion-gate.sh", "hermes-retry-check.sh",
    "hermes-error-dna.sh", "hermes-turn-counter.sh", "hermes-compact-detect.sh",
    "hermes-edit-scope.sh", "hermes-session-handoff.sh", "hermes-meta-oracle-trigger.sh",
    "pre-output-check.py",
]
...
def test_context_guard():
    ...
    # context-guard.sh 已迁移为 .py，不再检查 sh 版本
    log("PASS" if cg.exists() else "FAIL", "context-guard.py present (replaces .sh)")
...
def test_pretool_composite():
    print("\\n--- Pre-Tool Gates ---")
    # Hermes 系统脚本在 ~/.hermes/scripts/ 中
    for g in PRETOOL_GATES:
        exists(HERMES_SCRIPTS / g, f"{g} (hermes scripts)")
    # CarrorOS 自身 .py 门禁
    for g in PRETOOL_GATES_PY:
        exists(HOOKS_DIR / g, f"{g} (carroros hooks)")
    ...
    if sj.exists():
        ...
        hook_str = " ".join(hooks)
        # Hermes 系统脚本注册在 ~/.hermes/scripts/，不在 CarrorOS settings 中注册
        for g in PRETOOL_GATES:
            log("INFO", f"settings.json: {g} (hermes system script, not registered here)")
...
def test_gate_auth():
    print("\\n--- Gate Auth ---")
    # Hermes 门禁脚本在 ~/.hermes/scripts/（非 CarrorOS hooks 目录）
    for name in ["hermes-pre-exec.sh", "hermes-completion-gate.sh", "pre-output-check.sh",
                  "hermes-error-dna.sh", "hermes-retry-check.sh"]:
        exists(HERMES_SCRIPTS / name, f"{name} (hermes scripts)")
    pe = HERMES_SCRIPTS / "hermes-pre-exec.sh"
    if pe.exists():
        contains(pe, r"command|block|approve", "pre-exec has command processing")
...
def test_capability_matrix_runner():
    print("\n--- Cap Matrix Runner ---")
    exists(SCRIPTS_DIR / "capability-matrix-test.sh", "capability-matrix-test.sh")
    executable(SCRIPTS_DIR / "capability-matrix-test.sh", "capability-matrix-test.sh executable")
