...
| 平台 | 开发商 | 配置形式 | 覆盖 Hook |
|------|--------|---------|:---------:|
| **Claude Code** | Anthropic | `.claude/harness.yaml`（原生配置） | 19/19 |
| **Codex CLI** | OpenAI | `.codex/hooks.json`（自动生成） | 11/19 |
| **Gemini CLI** | Google | `.gemini/settings.json`（自动生成） | 11/19 |
| **Qwen Code** | 阿里巴巴 | `settings.json`（自动生成） | 11/19 |
| **Cursor** | Cursor Inc. | `.cursor/hooks.json`（自动生成） | 2/19 |
| **OpenCode** | 开源社区 | `.opencode/plugins/sylph-hooks.ts`（自动生成） | 5/19 |
...
## Event Support by Platform（平台事件覆盖明细）

```
  claude_code          11 hooks, 9/9 events
  codex                11 hooks, 9/9 events
  gemini               11 hooks, 7/9 events — missing: session:start, compact:before
  qwen                 11 hooks, 9/9 events
  cursor               2 hooks, 6/9 events — missing: session:start, file:write, compact:before
  opencode             5 hooks, 9/9 events
```
...
```
Claude Code  ████████████████████████████████  19 守卫（原生全量）
Codex CLI    ████████████████████              11 守卫（全事件）
Gemini CLI   ████████████████████              11 守卫（缺 2 事件）
Qwen Code    ████████████████████              11 守卫（全事件）
OpenCode     ████████                           5 守卫（可扩展）
Cursor       ███                                2 守卫（架构局限）
```
...
```
     ...
     (原生, 19/19)        (生成, 11/19)        (插件, 5/19)
          │                    │                    │
     Gemini CLI            Qwen Code             Cursor
     (生成, 11/19)         (生成, 11/19)        (生成, 2/19)
          │                    │                    │
          └────────────────────┼────────────────────┘
                               │
                    ┌──────────┴──────────┐
