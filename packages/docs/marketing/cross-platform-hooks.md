...
| **Gemini CLI** | Google | `.gemini/settings.json`（自动生成） | 19/29 |
| **Qwen Code** | 阿里巴巴 | `settings.json`（自动生成） | 19/29 |
| **Cursor** | Cursor Inc. | `.cursor/hooks.json`（自动生成） | 4/29 |
...
```
  ...
  gemini               19 hooks, 7/9 events — missing: session:start, compact:before
  qwen                 19 hooks, 9/9 events
  cursor               4 hooks, 6/9 events — missing: session:start, file:write, compact:before
