...
EOF
    ...
    echo "$stdin" | python3 "$ERROR_DNA" 2>/dev/null || true
}
...
EOF
    ...
    echo "$stdin" | python3 "$BASH_AUDIT" 2>/dev/null || true
}

# Seed gate-block jsonl entries for E3/E4 detection
# Writes to BOTH error-dna.jsonl AND error-signals.jsonl — posttool-bash-audit.py
# reads signals file first with fallback to dna file. With signals present but
# without seeds, E3/E4 detection fails (ctx_blocks=0).
seed_gate_blocks() {
    ...
    for i in $(seq 1 $count); do
        case "$type" in
            context_guard)
                local entry="{\"tool\":\"Bash\",\"error_type\":\"context_limit\",\"ts\":$((ts - i * 5)),\"message\":\"context-guard blocked at ${i}% context\",\"cmd\":\"some-command-${i}\"}"
                echo "$entry" >> "$JSONL"
                echo "$entry" >> "$STATE_DIR/error-signals.jsonl"
                ;;
            soft_block)
                local entry="{\"tool\":\"Bash\",\"error_type\":\"gate_operation\",\"ts\":$((ts - i * 5)),\"message\":\"completion-gate soft-block #${i}: evidence missing\",\"cmd\":\"some-command-${i}\"}"
                echo "$entry" >> "$JSONL"
                echo "$entry" >> "$STATE_DIR/error-signals.jsonl"
                ;;
            *)
                local entry="{\"tool\":\"Bash\",\"error_type\":\"${type}\",\"ts\":$((ts - i)),\"command\":\"blocked-cmd-${i}\"}"
                echo "$entry" >> "$JSONL"
                echo "$entry" >> "$STATE_DIR/error-signals.jsonl"
                ;;
        esac
...
# E1e: Normal harmless command — error-dna writes all bash commands to error-dna.jsonl,
# so 1 record is expected (not 0). No governance_bypass tag is correct.
echo "  Test E1e: harmless command (records all commands, no bypass tag)"
BEFORE=$(wc -l < "$JSONL")
run_hook_error_dna 0 "ls -la $PROJECT_ROOT/src" "$PROJECT_ROOT/src"
AFTER=$(wc -l < "$JSONL")
assert_eq "   记录追加（所有命令写 error-dna.jsonl）" "1" "$((AFTER - BEFORE))"
LAST=$(tail -1 "$JSONL")
...
# Note: cat (read) is IS_WRITE_CMD=False, so E1 bypass not triggered.
# Record is written with error_type='runtime', no governance_bypass tag.
...
# error-dna resolves symlinks via os.path.realpath() — /tmp/settings-link
# resolves to .claude/settings.json, which matches GOVERNANCE_PATHS.
# Note: cat (read) is IS_WRITE_CMD=False, so E1 bypass not triggered.
echo "  Test T2b: symlink to settings.json (read, no bypass)"
...
# T7a: gh CLI write operation — error-dna records all bash commands,
# but gh isn't matched as governance_bypass (not in E1_WRITE_PATTERN).
# The record goes to error-dna.jsonl with error_type='runtime'.
echo "  Test T7a: gh release upload (recorded but no bypass)"
BEFORE=$(wc -l < "$JSONL")
