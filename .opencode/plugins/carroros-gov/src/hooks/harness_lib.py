...
def hc_fail_closure(group="logging_hooks"):
    """Check fail-close policy for a group.
    
    读取 fail_closure.{group} 配置: 'close' → 应 fail-close, 'open' → fail-open。
    默认: 'open' (fail-open 安全)。
    """
    val = hc_get(f"fail_closure.{group}", "open")
    return val.strip().lower() == "close"
