...
def is_mode_active(state_dir=None):
    """检测 ghost/goal 模式是否激活（返回 ghost/goal/normal）"""
    if state_dir is None:
        state_dir = _HC_STATE_DIR
    state_dir = str(state_dir)
    now_epoch = time.time()

    # 检查 lx-ghost.json（新格式）
    ghost_new = os.path.join(state_dir, "tokens", "lx-ghost.json")
    if os.path.isfile(ghost_new):
        try:
            with open(ghost_new, 'r', encoding='utf-8') as f:
                d = json.load(f)
    ...
    # 旧格式 ghost-mode.json
    ghost_old = os.path.join(state_dir, "ghost-mode.json")
    if os.path.isfile(ghost_old):
        try:
            with open(ghost_old, 'r', encoding='utf-8') as f:
                d = json.load(f)
            expires = d.get('expires_at', '')
            if not expires:
                return "ghost"
    ...
    # ghost-mode.active
    if os.path.isfile(os.path.join(state_dir, "ghost-mode.active")):
        return "ghost"

    # lx-goal.json（新格式）
    goal_new = os.path.join(state_dir, "tokens", "lx-goal.json")
    if os.path.isfile(goal_new):
        try:
            with open(goal_new, 'r', encoding='utf-8') as f:
                d = json.load(f)
            expires = d.get('expires_at', '')
            if not expires:
                return "goal"
    ...
    # unattended-mode.json（旧格式）
    goal_old = os.path.join(state_dir, "unattended-mode.json")
    if os.path.isfile(goal_old):
        try:
            with open(goal_old, 'r', encoding='utf-8') as f:
                d = json.load(f)
            expires = d.get('expires_at', '')
            if not expires:
                return "goal"
    ...
    # .unattended-mode
    if os.path.isfile(os.path.join(state_dir, ".unattended-mode")):
        return "goal"
...
def _mode_append_to_list(state_dir, mode, field, json_value):
    """原子追加 JSON 值到模式状态文件的列表字段"""
    mode_files = {
        "ghost": os.path.join(state_dir, "tokens", "lx-ghost.json"),
        "goal": os.path.join(state_dir, "tokens", "lx-goal.json"),
        "unattended": os.path.join(state_dir, "unattended-mode.json"),
    }
    file = mode_files.get(mode, os.path.join(state_dir, f"{mode}-mode.json"))
...
def main():
    ...
    if not stdin_data:
        # 无输入 → fail-closed
        print("⛔ [Permission Gate] 无法读取输入 — 安全门禁默认阻断。", file=sys.stderr)
        hc_emit_hook_json("PreToolUse", False,
                          "[Permission Gate] 输入读取失败，安全门禁默认阻断。请人类在终端手动执行此命令，或明确确认放行。")
        sys.exit(2)
    ...
    # 命令提取失败 → fail-closed
    if not command:
        print("⛔ [Permission Gate] 无法解析命令文本 — 安全门禁默认阻断。请在终端手动执行或确认命令。", file=sys.stderr)
        hc_emit_hook_json("PreToolUse", False,
                          "[Permission Gate] 命令提取失败，安全门禁默认阻断。请人类在终端手动执行此命令，或明确确认放行。")
        sys.exit(2)
