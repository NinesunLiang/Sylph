...
2. Owner 查看 pending decisions
    → 运行: lx-prd-mothership status
    → 查看 open CONFLICT 列表
    → 运行: lx-prd-mothership resolve CONFLICT-NNN

3. 裁决命令格式
    lx-prd-mothership resolve <CONFLICT-ID> <verdict> [--reason "说明"]
...
执行裁决: lx-prd-mothership resolve CONFLICT-001 accept|reject|defer [--reason "说明"]
```
...
2. `propagate` 对每个目标 feature 执行前，先读取 `features/feat-xxx/sync-notes.md`
...
- Propagated By: lx-prd-mothership
...
| `sync-notes.md` | lx-prd-mothership（init 时） | 记录母本向本 feature 的增量同步历史（含 CHG-ID） |
...
> 注意：这是预览模式，未实际写入任何文件。
> 执行实际传播请运行: lx-prd-mothership propagate --execute
...
2. 运行: lx-prd-mothership deprecate FEAT-xxx [--reason "说明"]
3. 系统操作：
   - 在 features/feat-xxx/prd.md 顶部插入 DEPRECATED 声明
   - 在 state/feature-map.md 中标记 FEAT-xxx status: deprecated
   - 在 PHASES.md 中更新对应 phase 的 feature 列表
   - 生成废弃快照: snapshots/features/feat-xxx-deprecated-YYYYMMDD.md
...
> 本文件为 lx-prd-mothership spec-v0 的补丁，两者共同构成 SKILL.md 编写的完整前置规范。