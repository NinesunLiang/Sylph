...
### 6. A→B→A 交叉验证（方案/验收类任务）

方案/报告/验收类任务必须经过双终端交叉验证：
...
- 触发条件：completion-gate 检测 evidence 含「报告/方案/验收/通过率/评估/标准」等关键词时，打印交叉验收提醒
- 提醒非阻断（exit 0），由用户决定是否执行 A→B→A
- ⚠️ A 与 B 必须使用不同模型（如 Claude + GPT / Gemini / DeepSeek），同模型盲区重叠，交叉验证效果有限
- 铁律 #7（断言真实）为硬约束，与交叉验证提醒互补
