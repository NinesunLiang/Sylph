...
用法:
  python3 scripts/test-coverage-gate.py           # 扫描+报告
  python3 scripts/test-coverage-gate.py --block    # 无测试覆盖则 exit 2
...
TEST_SCRIPTS_DIR = PROJECT_ROOT / "scripts"
...
def main() -> int:
    ...
    if uncovered:
        ...
        print()
        print(f"  测试文件名模式: scripts/test-<mechanism>.py 或 .claude/hooks/tests/test-<mechanism>.py")
        print()
        if block:
