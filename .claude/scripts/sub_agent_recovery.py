...
class RecoveryCheckpoint:
    ...
    def _sub_task_check(self) -> dict:
        ...
        if token_path.exists():
            try:
                ...
                # The manager writes parent.task_dir / parent.plan_id and
                # subtask.plan_text. Read both new and legacy keys so resume
                # output keeps parent identity and plan regardless of writer.
                parent = token.get("parent", {}) or {}
                subtask = token.get("subtask", {}) or {}
                result["parent_id"] = (
                    parent.get("task_id")
                    or parent.get("task_dir")
                    or parent.get("plan_id")
                )
                result["subtask_plan"] = (
                    subtask.get("plan_text")
                    or subtask.get("plan")
                    or ""
                )
    ...
    def generate_resume(self) -> str:
        ...
        # 续跑指引（P2-7, index12 S4）：有未完成工作时给出可执行的 Next Action
        next_actions: list[str] = []
        if status["type"] == "task":
            pending = status.get("pending_steps", [])
            if pending:
                next_actions.append(f"继续执行下一个待办步骤: {pending[0]}")
                for step in pending:
                    next_actions.append(f"- [ ] {step}")
        else:
            if status["status"] not in ("completed", "cancelled", "failed", "timeout"):
                next_actions.append(
                    f"继续/重试步骤 {status.get('step_id')}（当前状态 {status['status']}）"
                )
        if next_actions:
            lines.extend(["## Next Action", ""] + next_actions + [""])
    ...
    def save_continuation(self, output_path: Path = None) -> dict:
        """生成结构化续跑产物并标记 continuation_attempted。

        P2-7 (index12 S4)：使 compact 恢复不再是 summary-only——产出
        state/continuation.json，供新 agent 拿到可执行的下一步（resume→continue→verify）。
        """
        status = self.check_status()
        if status["type"] == "task":
            pending = status.get("pending_steps", [])
            next_step = pending[0] if pending else None
            continuation_attempted = bool(pending)
        else:
            terminal = status["status"] in ("completed", "cancelled", "failed", "timeout")
            next_step = None if terminal else status.get("step_id")
            continuation_attempted = not terminal

        info: dict = {
            "task_dir": str(self.task_dir),
            "step_id": self.step_id,
            "status": status["status"],
            "pending_steps": status.get("pending_steps", []) if status["type"] == "task" else [],
            "next_step": next_step,
        ...
        if output_path is None:
            ...
            output_path = state_dir / "continuation.json"
