...
**激活命令**（必须用 Bash 工具执行，不可手动 touch）：
```
bash .claude/skills/lx-goal/scripts/lx-goal.sh on "{目标描述}"
```

此命令创建两个信号文件：
- `.omc/state/lx-goal.json` — `is_mode_active()` 读取此文件判断是否为 goal 模式
- `.omc/state/autonomous.active` — completion-gate / pre-completion-gate 等 hook 据此降级（context-guard 通过 is_mode_active() 读取 lx-goal.json，间接生效）

**激活后验证**（激活后立即执行，确认信号文件存在）：
```
ls -la .omc/state/lx-goal.json .omc/state/autonomous.active
```

**为什么必须走脚本**：手动 `touch autonomous.active` 只创建一个文件，`is_mode_active()` 读取 `lx-goal.json` 而非 `autonomous.active`，半个系统仍在 normal mode，agentic_menu 照弹不误（DG-46 教训）。

**激活后**：持续执行子任务，逐项标记 task-done，直到全部完成或过期。所有 hook 的 MODE guard 现在生效，交互弹窗被物理阻断。
...
### ⚠️ 需人为决策汇总（REQUIRED — 聚合所有需人类关注项）

> **此节必须出现在每个退出报告中，作为第一个"需人类关注"区块。**
> 将所有需要人类审查的项跨类别聚合，避免人类逐节翻找。
