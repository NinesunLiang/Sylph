...
### Phase 0：前置澄清（支持 standard / batch 双模式）
...
| 模式 | 适用 | 说明 |
|:----|:----|:-----|
| **standard**（默认） | 一般任务 | 一次一问，逐分支推进（同 Base）|
| **batch** | 高阶模型/复杂任务 | 一轮问完所有前沿问题，frontier ≤ 5，按依赖排序 |

Batch 模式规则：
1. 构建设计树：每个决策标记依赖关系
2. 每轮 frontier = 所有依赖已解决的决策
3. 每轮最多 5 个问题，按依赖排序，每个问题附"为什么问这个"
4. sub-agent 查不同步阻塞，但产出结论前同步等待（sync barrier）
5. frontier 为空时完成
...
2. **模式分发** — 自动选择模式：
   ...
   ```
   任务复杂度为"复杂" 且 模型为高阶(Opus/GPT/Grok) → batch
   其他情况 → standard
   ```

   用户也可通过 `--mode batch|standard` 强制指定。
3. **判断任务复杂度**：
...
6. **自审计划** — 检查占位符/矛盾/模糊项
7. **ADR 判断** — 逐项检查三条件，全部满足才产出：

   ```
   条件1: 难撤销吗？ → 这个决定改了之后，后悔成本高吗？是/否
   条件2: 非直觉吗？ → 未来读者会问"为什么这么干"吗？是/否
   条件3: 真权衡吗？ → 有明确的替代方案并做了选择吗？是/否
   
   三条件全是 → 产出 ADR（.claude/references/adr/NNNN-title.md）
   任一否   → 跳过，不产出
   ```

   产出后刷新 `.claude/references/adr/INDEX.md`。

8. **Phase0 完成判定** — 满足以下所有条件才算完成：
   - 所有不确定项已扫描
   - 已分发到标准或 batch 模式
   - 复杂任务已产方案对比（简单任务跳过）
   - 计划已自审
...
> HARD-GATE 不跳过。Enhance 不需 spec 模板格式，对话式记录即可。
