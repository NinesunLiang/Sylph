...
### Step 1: 解析审查目标
加载 `@../../nodes/target_resolver.md`，传入 `$ARGUMENTS`。- 过滤：保留 `*.go`，排除 `*_test.go`、`vendor/`、`*.pb.go`、`mock_*.go`、`*_gen.go`、`testdata/`
...
### Step 4: 误报排除
**误报场景**（标记为 FP，不报告）：- 在注释或字符串中- 有显式 `//nolint` 且理由合理- error 已通过其他路径处理- go-zero 生成的模板代码（goctl 产出）- H1 中的内部函数（非公开）→ 降级为 P3- H2 中仅读取不做分支判断 → 标记 "待确认"
不确定 → 标记 "待确认: [reason]"。
...
```markdown
##
[Code Review 反哺] {规则号} {问题简述}- **文件**: `{file:line}`- **问题**: {问题描述}- **改进**: {修复方式}- **来源**: lx-code-review auto-fix- **严重度**: {P0 🔴 / P1 🟠}
