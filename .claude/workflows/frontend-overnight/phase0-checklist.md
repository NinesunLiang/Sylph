...
- [ ] **A2 夜跑 hook 挂载**：`bash scripts/carroros-gates/install-night-hook.sh`（幂等）
...
- [ ] **A4 §17a**：把落盘 diff 发 Opus 4.8 / Grok 4.5 / GPT-5.6 Sol 审计，无新阻断性 P0
  - [ ] 附**独立复跑日志**（Grok P0-2）：rsync 仓库到 /tmp 干净目录 → `SMOKE_RUNNER=independent bash scripts/carroros-gates/smoke/run-all.sh ...` 全绿 → 日志落 `UI/round5/logs/`（self 自陈不得作为首夜放行证据）
...
- [ ] **B5 控制面锁**：`bash scripts/carroros-gates/gen-control-plane-lock.sh --manifest .omc/night/{date}/night-manifest.yaml --write`
...
- [ ] **B8 preflight**：
  ```bash
  bash scripts/carroros-gates/preflight.sh --manifest .omc/night/{date}/night-manifest.yaml \
