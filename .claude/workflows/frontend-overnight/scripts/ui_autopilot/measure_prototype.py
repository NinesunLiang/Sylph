"""measure_prototype.py — Extract computed styles from prototype pages via Playwright.

Produces measurements dict per region:
  {
    "region_id": {
      "bbox": {x, y, width, height},
      "colors": ["#hex1", "#hex2", ...],
      "fonts": [{"family": "Inter", "size": 16, "weight": 400, ...}],
      "borders": [{radius: 8, width: 1, style: "solid", color: "#..."}],
...
Usage:
  python3 measure_prototype.py --url http://localhost:3000/prototype \
...
def measure_prototype(url: str, regions: list[dict[str, Any]]) -> dict[str, Any]:
    """Extract measurements from prototype page.

    Args:
        url: Prototype page URL
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Extract prototype measurements")
    parser.add_argument("--url", required=True, help="Prototype page URL")
    ...
    measurements = measure_prototype(args.url, regions)
