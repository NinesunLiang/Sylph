...
# 5. 模型路由（新方案：adapter 8765 + 线上模型）
ADAPTER_URL = "http://127.0.0.1:8765"
base_url = os.environ.get("ANTHROPIC_BASE_URL", "")
if base_url and base_url != ADAPTER_URL:
    note(f"ANTHROPIC_BASE_URL={base_url}（期望 {ADAPTER_URL}）")
else:
    try:
        import json as _json
        r = subprocess.run(["curl", "-s", "-m", "5", ADAPTER_URL + "/health"], capture_output=True, text=True, timeout=10)
        if r.returncode == 0:
            data = _json.loads(r.stdout)
            if data.get("session_valid"):
                ok(f"adapter在线({ADAPTER_URL}) models={len(data.get('models',[]))}")
            else:
                note(f"adapter session过期({ADAPTER_URL})")
        else:
            note(f"adapter离线({ADAPTER_URL})")
    except Exception as e:
        note(f"adapter不可达({ADAPTER_URL}): {e}")
...
r = subprocess.run([sys.executable, str(GATES_DIR / "smoke" / "run_all.py"),
