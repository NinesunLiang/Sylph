...
|`context-guard.py` | PreToolUse | ─── 导入共享库 ───|
...
|`edit-guard.py` | PreToolUse | Import shared library|
...
|`fuzzy-block.py` | PreToolUse | ─── 导入共享库 ───|
...
|`lsp-suggest.py` | PreToolUse | ── Import harness shared library ──────────────────────────────────|
...
|`permission-gate.py` | PreToolUse | permission-gate.py — PreToolUse:Python — 执行危险命令前检查权限申请格式|
...
|`pre-ask-guard.py` | PreToolUse | ─── Import shared library ───|
|`pre-ask-guard` | PreToolUse | pre-ask-guard.sh — PreToolUse:AskUserQuestion — 问人前强制过决策链四层评估|
|`pre-completion-gate.py` | PreToolUse | Import shared library|
...
|`pretool-edit-scope.py` | PreToolUse | ── Import shared library ──|
...
|`pretool-plan-gate.py` | PreToolUse | pretool-plan-gate.py — PreToolUse:Edit|
...
|`pretool-retry-check.py` | PreToolUse | ── 引入同级共享库 ──|
...
|`pretool-sensitive-edit.py` | PreToolUse | 添加 hooks 目录到 sys.path，以便导入 harness_lib|
...
|`subagent-guard.py` | PreToolUse | 添加 hooks 目录到 sys.path，以便导入 harness_lib|
|`thinking-gate` | UserPromptSubmit | thinking-gate.sh — Thinking Content 上下文门禁|
