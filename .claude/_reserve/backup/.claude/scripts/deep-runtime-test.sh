...
# Helper: run hook (.py preferred)
_hook_run() {
    local base="$1"
    if [ -f "$H/${base}.py" ]; then python3 "$H/${base}.py"
    else echo "ERROR: $H/${base}.py not found"; return 1; fi
}
# Helper: grep hook file with extended regex (-E)
_hook_grep() {
    local base="$1" pattern="$2"
    local file="$H/${base}.py"
    [ -f "$file" ] && grep -cE "$pattern" "$file" 2>/dev/null || echo 0
}
...
LSP_PY=$(echo '{"tool_input":{"file_path":"test.py"}}' | (_hook_run pre-edit-lsp-check 2>&1 || echo ""))
_t "pre-edit-lsp .py responds" "continue" "$LSP_PY"

# 1.3 pre-edit-lsp 对 .md 的跳过
LSP_MD=$(echo '{"tool_input":{"file_path":"readme.md"}}' | (_hook_run pre-edit-lsp-check 2>&1 || echo ""))
_t "pre-edit-lsp skips .md" "continue" "$LSP_MD"

# 1.4 lsp-suggest 对 CamelCase 的反应 (output is JSON: {"continue": true})
LSP_SUG=$(echo '{"tool_input":{"pattern":"TaskRunner"}}' | (_hook_run lsp-suggest 2>&1 || echo ""))
_t "lsp-suggest triggers on CamelCase" "continue" "$LSP_SUG"

# 1.5 lsp-suggest 对小写的跳过
LSP_SKIP=$(echo '{"tool_input":{"pattern":"task"}}' | (_hook_run lsp-suggest 2>&1 || echo ""))
...
# 2.1 哲学优先级链存在
PHILO_FILE=".claude/reference/philosophy.md"
[ -f "$PHILO_FILE" ] || PHILO_FILE=".claude/philosophy.md"
_t "philosophy priority: 4>6>3" "[4-9]" "$(grep -cE '#4|#6|#3' "$PHILO_FILE" 2>/dev/null || echo 0)"

# 2.2 决策矩阵覆盖核心场景
MATRIX_LINES=$(wc -l < .claude/reference/autonomous-decision-chain.md 2>/dev/null || echo 0)
_t "decision chain doc lines >= 40" "[4-9][0-9]" "$MATRIX_LINES"

# 2.3 DG-91: Oracle REVISE → 直接修 (不问"要修吗")
_t "DG-91 encoded" "[1-9]" "$(grep -cE 'DG-91|直接修.*不问|REVISE.*fix immediately' .claude/claude-next.md 2>/dev/null)"

# 2.4 铁律#8 哲学先行执行协议
AUTOD_FILE=".claude/reference/autonomous-decision-chain.md"
_t "iron-rule #8 philosophy-first protocol" "[1-9]" "$(grep -cE '哲学先行|#8.*执行' "$AUTOD_FILE" 2>/dev/null)"

# 2.5 Claim-audit: 无 file:line 断言应被拦截
_t "claim-audit hook active" "[1-9]" "$(_hook_grep posttool-claim-audit 'file:line|双源')"

# 2.6 反模式 F1 (假设驱动) 检测
_t "anti-pattern F1 detection" "[1-9]" "$(_hook_grep posttool-anti-pattern-detect 'F1.*假设|应该是|possibly')"
...
_t "OMA orch has pipeline steps" "[1-9]" "$(grep -ciE 'Step|phase|stage' .claude/skills/lx-oma-orch/SKILL.md 2>/dev/null || echo 0)"

# 3.5 OMA split MECE validation
_t "OMA split has MECE logic" "[1-9]" "$(grep -ciE 'MECE|正交|interface.*contract' .claude/skills/lx-oma-split/SKILL.md 2>/dev/null)"
...
# 4.2 硬边界协议（内容在 references/body.md）
_t "hard boundary protocol" "[1-9]" "$(grep -ciE '硬边界|hard.boundary' .claude/skills/lx-goal/references/body.md 2>/dev/null)"

# 4.3 三级裁决链（内容在 references/body.md）
_t "3-level decision chain" "[1-9]" "$(grep -ciE 'Level [123]|三级|3.*level' .claude/skills/lx-goal/references/body.md 2>/dev/null)"

# 4.4 卡点分类处理矩阵（内容在 references/body.md）
_t "blocking classification matrix" "[1-9]" "$(grep -ciE '卡点类型|硬边界|可跳过|可绕行|真阻断' .claude/skills/lx-goal/references/body.md 2>/dev/null)"

# 4.5 Goal mode gate degradation
_t "permission-gate degrades in goal" "[1-9]" "$(_hook_grep permission-gate 'is_mode_active|autonomous')"
...
# 4.8 退出报告模板
_t "goal exit report template" "[1-9]" "$(grep -ciE '退出报告|exit report|需人为决策' .claude/skills/lx-goal/SKILL.md 2>/dev/null)"
