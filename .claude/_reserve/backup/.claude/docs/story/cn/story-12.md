...
ED-01 是这个门禁的典型案例——Error-DNA 和 Build-Validator 通过了"存在且运行"的形式检查，但没有通过"实际产生价值"的收益检查。83.5% 噪声率、0 次 repair_success、40 条无人消费的 build 记录——按机制采纳门禁的三问判定为：移除。
...
从 v6.1.9 到 v6.3.8，每一次版本号增加，都有对应的 claude-next.md R 条目或 DG 条目作为来源。这些条目不是某个人凭空想出来的优化方向——它们是狗粮模式下的真实踩坑、真实修复、真实机制化。
