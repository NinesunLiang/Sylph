...
def main():
    ...
    if not cruising_file.exists():
        ...
        # Try to extract feature from mode json (new format only)
        lx_goal_file = state_dir / "tokens" / "lx-goal.json"
        lx_ghost_file = state_dir / "tokens" / "lx-ghost.json"

        if lx_goal_file.exists():
            try:
                import json as jmod
                data = jmod.loads(lx_goal_file.read_text(encoding="utf-8", errors="replace"))
                feature_hint = data.get("goal", "unknown")
            except Exception:
                pass
        elif lx_ghost_file.exists():
            try:
                import json as jmod
                data = jmod.loads(lx_ghost_file.read_text(encoding="utf-8", errors="replace"))
                feature_hint = data.get("direction", "unknown")
            except Exception:
                pass
