...
用法:
  python3 .claude/references/feature_test/test-coverage-gate.py           # 扫描+报告
  python3 .claude/references/feature_test/test-coverage-gate.py --block    # 无测试覆盖则 exit 2
...
FEATURE_TEST_DIR = PROJECT_ROOT / ".claude" / "references" / "feature_test"
...
MECHANISMS: list[tuple[str, str, list[str], str | None]] = [
    ...
    ("edit-guard", "hook", [".claude/hooks/edit-guard.py"], None),
    ("context-guard", "hook", [".claude/hooks/context-guard.py"], "test-context-watermark.py"),
    ("privacy-gate", "hook", [".claude/hooks/privacy-gate.py"], None),
    ("pre-ask-guard", "hook", [".claude/hooks/pre-ask-guard.py"], None),
    ("pretool-approve-detect", "hook", [".claude/hooks/pretool-approve-detect.py"], None),
    ("pretool-user-approve", "hook", [".claude/hooks/pretool-user-approve.py"], None),
    ("pretool-blast-radius", "hook", [".claude/hooks/pretool-blast-radius.py"], None),
    ("pretool-terminal-safety", "hook", [".claude/hooks/pretool-terminal-safety.py"], None),
    ("pretool-purify-gate", "hook", [".claude/hooks/pretool-purify-gate.py"], None),
    ("pretool-write-lock", "hook", [".claude/hooks/pretool-write-lock.py"], None),
    ("permission-gate", "hook", [".claude/hooks/permission-gate.py"], None),
    ...
    ("posttool-gate", "hook", [".claude/hooks/posttool-gate.py"], None),
    ("posttool-checkpoint", "hook", [".claude/hooks/posttool-checkpoint.py"], None),
    ...
    ("session-end-lifecycle", "hook", [".claude/hooks/session-end-lifecycle.py"], None),
    ("precompact-lifecycle", "hook", [".claude/hooks/precompact-lifecycle.py"], "test_pkg_c_lifecycle.py"),
    ("subagent-stop-lifecycle", "hook", [".claude/hooks/subagent-stop-lifecycle.py"], None),
...
def main() -> int:
    ...
    if uncovered:
        ...
        print()
        print(f"  测试文件名模式: .claude/references/feature_test/test-<mechanism>.py 或 .claude/references/feature_test/test-<mechanism>.sh")
        print()
        if block:
