...
**模型与代理一致性：**
```text
1. L1/L2 是治理等级，不是模型等级。
2. SubAgent 默认继承 MainAgent 的模型配置；所有 SubAgent 使用相同模型策略。
3. MainAgent 与 SubAgent 共用相同 scope、权限、验证、证据和完成标准。
4. 禁止根据 deepseek/qwen/opus/gpt 等模型名称静默改变治理语义。
5. 异构模型只能作为显式实验或人工批准的例外，并必须记录路由原因与验证结果。
```
