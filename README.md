...
**产品**simpleX| **技术栈**：Vite 8 · React 19 · TS 6 strict · Sass+CSS Modules · React Router v7 · Zustand v5 · Axios v1
...
**哲学优先级**（前 3 条）：**#4 验证 > #6 零信任 > #3 守护**
**权威链**：人类指令 > 哲学铁律 > PRD > 设计文档 > 代码
**人类不在线**（30 分钟无回应）：走最小风险路径（不删不改不发布），暂停等。

**哲学速查**：#1 less(少即是多) #2 增益(每步增值) #3 守护(不引入破坏) #4 验证(做完必验) #5 人(人类裁决) #6 零信任(独立验证) #7 文档(留痕可追溯)
