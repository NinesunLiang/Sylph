# Carror OS — 全特性参考手册 (v6.1.9 Cross-Platform Edition)
...
* **/lx-varlock**：企业级 DLP 隐私脱敏代理管理器。允许 AI 在脱敏占位符的环境下进行密码交互与命令执行，双向透明混淆，防止泄密。 **(用法：`varlock.py set KEY val` -> `varlock.py read .env`)**
...
| 文件 | 说明 |
| :--- | :--- |
| **`../tests/cn/auto-feature-test.md`** | 验收执行手册。对 AI 说"请帮我执行战区一的测试"即可启动，无需手动敲命令。涵盖 Agentic UI 门禁体验、图表化可观测性、OMA 并发引擎等核心特性。 |
| **`../tests/cn/auto-feature-test-log.md`** | 验收战报模板。边测试边记录实际表现，最终签字存档。 |

> 💡 **新成员首次上手**：直接打开 `../tests/cn/auto-feature-test.md`，按头部三步引导操作，整个过程以 AI 全自动代跑为主，你只需要在弹出的交互表单里做选择。
...
| 文件 | 说明 |
| :--- | :--- |
| **`../tests/cn/manual-acceptance-test.md`** | 全人工验收清单，共 **49 项**，覆盖 harness-kit 全部 32 个底层 Hook 及所有核心 Skill。每一条命令你亲自执行，每一个结果你亲眼确认。Agentic UI 的拦截弹窗由 Hook 真实触发，而非 AI 模拟。 |
| **`../tests/cn/manual-acceptance-test-log.md`** | 对应战报模板，49 行空白记录表，Fail 项必须填写根因与修复方案，验收官签字后存档。 |
