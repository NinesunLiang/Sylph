...
*走廊尽头，posttool-anti-pattern-detect [注: 此机制在 v7.x 评估 ROI 不足，已移除 — 见 clean-dead-code-20260721] 的扫描光束扫过最新的输出——"应该没问题了"出现在第七行。扫描仪亮起红灯。*
...
### posttool-anti-pattern-detect [注: 此机制在 v7.x 评估 ROI 不足，已移除 — 见 clean-dead-code-20260721] — 反模式扫描仪

在每次 TaskUpdate/Edit/Write 后，posttool-anti-pattern-detect [注: 此机制在 v7.x 评估 ROI 不足，已移除 — 见 clean-dead-code-20260721] 扫描输出中的反模式信号：
...
这就是反模式 H1（语义编造）的诞生。裁判庭从这次事故中学会了：**形式合规 ≠ 语义真实。** 第四个反模式检测维度（H1）被加入 posttool-anti-pattern-detect [注: 此机制在 v7.x 评估 ROI 不足，已移除 — 见 clean-dead-code-20260721] 的扫描清单。
