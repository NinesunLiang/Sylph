...
操作系统从来不会强迫用户一开机就去学习复杂的系统调用。它应该默默地在后台管理内存和进程。
...
**适合**：只想给 AI 加一层物理防呆锁，不想看到任何复杂配置的极简主义者。
...
| `lx-oma-hier` | 分层 PRD 拆解 — 将超大型 PRD 按功能域拆分为 Sub PRD | 后台静默运行 / 主动调用 |
...
Carror OS 的架构解耦，你可以随时在三个层级之间切换，底层 Hooks 永不掉线且无损：
...
| 文件 | 说明 |
|:-----|:-----|
| `docs/tests/auto-feature-test.md` | 验收执行手册。对 AI 说"请帮我执行战区一的测试"即可启动 |
| `docs/tests/auto-feature-test-log.md` | 验收战报模板，边测试边记录 |

### 全人工验收（正式交付、安全审计、Zero Trust）

| 文件 | 说明 |
|:-----|:-----|
| `docs/tests/manual-acceptance-test.md` | 49 项全人工验收清单，覆盖全部 32 个 Hook 及核心 Skill |
| `docs/tests/manual-acceptance-test-log.md` | 对应战报模板，Fail 项必须填写根因与修复方案 |

### 终极审判（Dogfooding 前置）

| 文件 | 说明 |
|:-----|:-----|
| `docs/tests/final-exam.md` | 终极人工审判清单，零信任原则，每一项亲自执行才算数 |
