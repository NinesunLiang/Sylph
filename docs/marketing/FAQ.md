...
**Q: Hook 是怎么工作的？**
A: Carror OS 的 30 个 Hook 利用 Claude Code / OpenCode 的底层 Hook 协议，在 AI 的每次工具调用前插入拦截逻辑。当 AI 调用 `ExecuteCommand` 或 `WriteToFile` 等危险工具时，Hook 脚本在毫秒级执行检查。如果命中规则（如 `rm -rf`、明文密钥、超上下文阈值），Hook 返回 `Exit 2` 阻止操作。AI 不是"被建议不要做"，而是"物理上做不到"。
...
```
Base 版包含 30 个 Hook + 7 个门禁 Skill，零学习成本。

**Q: Base 版和 Enhanced 版的区别？**
A: Base 版 = 30 个 Hook（物理防线）+ 7 个自动化审查 skill（被动触发，全自动运行），零学习成本。Enhanced 版 = Base 全部能力 + 16 个主动式工作流 skill（RPE 流水线、根因分析、DLP 代理等），需要主动学习和调度。
...
A: Cursor Rules 和 Claude Code 原生 Hooks 提供的是原语（primitives），不是治理框架。Carror OS 是一个完整的有状态操作系统：30 个互锁的 Hook + 错误 DNA 记忆 + 甜点区主动交接 + A/B 盲审对抗 + DLP 双向脱敏。它不是几个规则的集合，而是一个分层治理体系。
...
**Q: Carror OS 经过哪些测试？**
A: L1-L4 四层测试体系（手动验收 + 自动 Hook 校验 + 代码扫描 + 格式门禁）98 项 98P/0F/4SOFT，通过 ShellCheck/Bandit 真实安全扫描（0 真实业务缺陷），对照 OWASP ASVS v4.0.3 / MITRE ATLAS / NIST AI RMF 1.0 三项行业标准完成合规对照（75/75 覆盖）。
