...
def cmd_verify(step_id=None):
    ...
    # ── Oracle 审查（仅 L2_ENHANCE）──
    if level == "L2_ENHANCE":
        try:
            import lib.phase3_oracle as oracle_mod
            plan_text = PLAN_PATH.read_text() if PLAN_PATH and PLAN_PATH.exists() else ""
            state = {"level": level, "task_id": token.get("session", {}).get("id", "")}
            o_result = oracle_mod.spawn_oracle(plan_text, state, evidence="",
                model_endpoint="http://127.0.0.1:9998/v1/messages", timeout=60)
            meta = o_result.get("meta", "") or ""
            verdict = "REJECT" if "REJECT" in meta else ("ADVISORY" if "ADVISORY" in meta else ("ACCEPT" if "ACCEPT" in meta else "ERROR"))
            print(_yellow(f"⚖️  Oracle 裁决: {verdict}"))
            if o_result.get("error"):
                print(_yellow(f"   Oracle 错误: {o_result['error'][:200]}"))
            if verdict == "REJECT":
                print(_red(f"❌ Oracle 审查不通过 (REJECT)"))
                return 2
        except ModuleNotFoundError:
            print(_yellow("⚠️ Oracle 模块不可用（lib.phase3_oracle 未安装），跳过增强审查"))
        except Exception as e:
            print(_yellow(f"⚠️ Oracle 运行异常: {e}，跳过增强审查"))
    verified_any = False
